...
describe('CheckboxWidget', () => {
    const defaultProps = {
        ...
        onFocus: jest.fn(),
        options: {},
        schema: { title: 'Test Checkbox', type: 'boolean' },
        uiSchema: {},
        value: false,
    };
    ...
    it('calls onChange with false when unchecked checkbox is clicked', async () => {
        const user = userEvent.setup();
        const handleChange = jest.fn();
        render(<CheckboxWidget {...defaultProps} onChange={handleChange} value={true} />);
        const checkbox = screen.getByTestId('test-checkbox');
    ...
    it('displays help text from schema description', () => {
        render(
            <CheckboxWidget
                {...defaultProps}
                schema={{ description: 'Help text', title: 'Test', type: 'boolean' }}
            />,
        );
    ...
    it('applies error class when there are errors', () => {
        render(<CheckboxWidget {...defaultProps} rawErrors={['Error message']} />);
