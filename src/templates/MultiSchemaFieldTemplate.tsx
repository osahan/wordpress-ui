...
const MultiSchemaFieldTemplate: React.FC<MultiSchemaFieldTemplateProps> = (props) => {
    const { optionSchemaField, schema, selector, uiSchema, ...rest } = props;
    ...
    useEffect(() => {
        if (
            ...
            onOptionChange &&
            selectedOption !== autoDetectedOptionIndex
        ) {
            ...
            {optionSchemaField && (
                <div className="rjsf-multi-schema-option-field">
                    {hasCustomWidget && fieldLabel ? (
                        <BaseControl label={fieldLabel}>{optionSchemaField}</BaseControl>
                    ) : (
                        optionSchemaField
                    )}
                </div>
