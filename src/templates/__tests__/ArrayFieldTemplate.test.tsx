...
describe('ArrayFieldTemplate', () => {
    ...
    const defaultProps = {
        ...
        readonly: false,
        required: false,
        schema: { items: { type: 'string' }, title: 'Test Array', type: 'array' },
        title: 'Test Array',
        uiSchema: {},
    };
    ...
    it('displays title from schema when title prop is not provided', () => {
        render(
            <ArrayFieldTemplate
                {...defaultProps}
                schema={{ title: 'Schema Title', type: 'array' }}
                title=""
            />,
        );
    ...
    it('renders in panel when usePanel is true', () => {
        const { container } = render(
            <ArrayFieldTemplate {...defaultProps} uiSchema={{ 'ui:options': { usePanel: true } }} />,
        );
    ...
    it('is disabled when disabled prop is true', async () => {
        const user = userEvent.setup();
        const handleAdd = jest.fn();
        render(<ArrayFieldTemplate {...defaultProps} disabled onAddClick={handleAdd} />);
        const addButton = screen.getByText(/Add Test Array/);
