...
describe('ObjectFieldTemplate', () => {
    const defaultProps = {
        ...
        properties: [
            {
                content: (
                    <div data-testid="property-1" key="prop1">
                        Property 1
                    </div>
                ),
            },
            {
                content: (
                    <div data-testid="property-2" key="prop2">
                        Property 2
                    </div>
                ),
            },
        ...
        readonly: false,
        required: false,
        schema: { title: 'Test Object', type: 'object' },
        title: 'Test Object',
        uiSchema: {},
    };
    ...
    it('displays title from schema when title prop is not provided', () => {
        render(
            <ObjectFieldTemplate
                {...defaultProps}
                schema={{ title: 'Schema Title', type: 'object' }}
                title=""
            />,
        );
    ...
    it('displays title from uiSchema when available', () => {
        render(
            <ObjectFieldTemplate {...defaultProps} title="" uiSchema={{ 'ui:title': 'UI Schema Title' }} />,
        );
    ...
    it('renders as simple div when no title', () => {
        render(<ObjectFieldTemplate {...defaultProps} schema={{ type: 'object' }} title="" />);
        expect(screen.getByTestId('property-1')).toBeInTheDocument();
    ...
    it('respects defaultOpen option', () => {
        const { container } = render(
            <ObjectFieldTemplate {...defaultProps} uiSchema={{ 'ui:options': { defaultOpen: true } }} />,
        );
